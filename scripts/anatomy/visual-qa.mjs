...
import { createHarness, project, setSlider } from './browser-qa.mjs';

const h=await createHarness({output:process.env.QA_OUTPUT_DIR||path.join(project,'docs/phase2.1/captures'),viewport:{width:1440,height:900}});
const {page,output,choose,reset,view,metrics,catalog}=h;
const report={date:new Date().toISOString(),source:'MED3D local production build',environment:'Headless Chromium, ANGLE SwiftShader; DPR 1. Human inspection required.',captures:[],errors:h.errors};
...
try{
  ...
  await group('skeletal:group:skull:neurocranium','inferior');await shot('06-craneo-inferior');
  ...
  await group('skeletal:group:leg-left:tarsals','superior');await zoomOut(1);await shot('12-pie','Left tarsal group with one zoom-out step.');
  ...
  ]){await reset();await group(id,direction);await explode('structures',100);await shot(name,'100% structure separation, deterministic hierarchical bounds.');}
...
}catch(error){report.error=error.stack;await page.screenshot({path:path.join(output,'visual-failure.png'),fullPage:false}).catch(()=>{});throw error;}
