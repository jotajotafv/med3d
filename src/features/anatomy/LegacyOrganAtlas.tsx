...
export default function LegacyOrganAtlas(){
 ...
 useEffect(()=>{
  setMounted(true);
  const params=new URLSearchParams(window.location.search),requestedOrgan=params.get('organ');
  const initialOrgan:OrganId=requestedOrgan==='lungs'||requestedOrgan==='brain'?requestedOrgan:'heart';
  const initialNodes=new Map(STRUCTURES[initialOrgan].map(node=>[node.id,node]));
  const requestedStructure=params.get('structure');
  const initialStructure=requestedStructure&&initialNodes.has(requestedStructure)?requestedStructure:null;
  const initialExpanded=new Set<string>([MODEL_DEFINITIONS[initialOrgan].root]);
  let ancestor=initialStructure?initialNodes.get(initialStructure):undefined;
  while(ancestor){initialExpanded.add(ancestor.id);ancestor=ancestor.parent?initialNodes.get(ancestor.parent):undefined;}
  setOrgan(initialOrgan);setExpanded(initialExpanded);setSelected(initialStructure);
  if(initialStructure){setPanel('details');setCameraRequest(previous=>({kind:'focus',id:initialStructure,version:previous.version+1}));}
 },[]);
 ...
 function changeOrgan(v:OrganId){setOrgan(v);setSelected(null);setHidden([]);setIsolated(null);setOpacity(1);setExploded(0);setQuery('');setExpanded(new Set([MODEL_DEFINITIONS[v].root]));request('reset');if(typeof window!=='undefined'){const u=new URL(window.location.href);u.searchParams.set('organ',v);u.searchParams.delete('structure');window.history.replaceState({},'',u)}}
 function choose(id:string){
  if(!byId.has(id))return;
  const selectedDescendants=new Set(descendants(organ,id));
  setHidden(previous=>previous.filter(hiddenId=>!selectedDescendants.has(hiddenId)&&!descendants(organ,hiddenId).includes(id)));
  setIsolated(previous=>previous&&descendants(organ,previous).includes(id)?previous:null);
  setExpanded(previous=>{const next=new Set(previous);let node=byId.get(id);while(node){next.add(node.id);node=node.parent?byId.get(node.parent):undefined;}return next;});
  setSelected(id);setPanel('details');request('focus',id);
  const url=new URL(window.location.href);url.searchParams.set('organ',organ);url.searchParams.set('structure',id);window.history.replaceState({},'',url);
 }
 ...
 <div className="atlas-heading"><div><span className="anatomy-eyebrow">EL CUERPO, PIEZA A PIEZA</span><h1>Explorador anatómico</h1></div><SiteLink href="/anatomia" className="atlas-edition">Volver al atlas del cuerpo</SiteLink></div>
