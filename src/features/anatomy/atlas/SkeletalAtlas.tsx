...
export default function SkeletalAtlas() {
  ...
  const education = current?.family?BONE_EDUCATION[current.family]:undefined;
  ...
  return <main className="atlas-page atlas-phase2">
    <div className="atlas-heading"><div><span className="anatomy-eyebrow">EL CUERPO, PIEZA A PIEZA</span><h1>Explorador anatómico</h1></div><span className="atlas-edition">ATLAS CORPORAL <span>·</span> FASE ÓSEA</span></div>
    {!catalog ? <section className="atlas-catalog-state" role={catalogError?'alert':'status'}><Bone size={36} weight="light"/><h2>{catalogError?'No se pudo abrir el atlas':'Preparando el catálogo anatómico'}</h2><p>{catalogError||'Las estructuras se cargarán por regiones.'}</p>{catalogError&&<button onClick={()=>setCatalogRetry(value=>value+1)}>Reintentar</button>}</section> : <>
      <div className="atlas-workspace">
        <aside className={'atlas-sidebar '+(panel==='structures'?'is-open':'')}>
          ...
          <label className="atlas-search"><MagnifyingGlass size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Nombre, sinónimo o término latino" aria-label="Buscar estructura anatómica"/>{query&&<button aria-label="Limpiar búsqueda" onClick={()=>setQuery('')}><X size={13}/></button>}</label>
          ...
          {query ? <div className="atlas-tree atlas-global-results"><p className="atlas-result-count" role="status">{found.length+legacyFound.length} resultados en el atlas disponible</p>
            {found.slice(searchPage*60,(searchPage+1)*60).map(node=><button className={'atlas-search-result '+(node.id===selected?'is-selected':'')} key={node.id} onClick={()=>choose(node.id)}><span>{node.name}</span><small>{node.latin||node.anatomicalName}</small><small>{regionName(node.regionId)}</small></button>)}
            ...
            {registeredSystems.map(system=>{
              ...
              return <section className="atlas-layer-system" key={system.id}>
                ...
                <div className="atlas-region-layers">{assets.map(asset=><label key={asset.id}><input type="checkbox" checked={assetIds.includes(asset.id)} onChange={()=>toggleAsset(asset.id)}/><span>{regionName(asset.regionId)}</span><small>{mb(asset.bytes)} MB</small></label>)}</div>
              </section>;
            })}
            <details className="atlas-planned"><summary>Otros sistemas del cuerpo</summary><p>Jerarquías previstas. Se incorporarán después de verificar el sistema óseo.</p>{SYSTEMS.filter(system=>!registeredSystems.some(item=>item.id===system.id)).map(system=><div key={system.id}><strong>{system.name}</strong><span>{system.branches}</span><small>Pendiente de incorporación corporal</small></div>)}</details>
            <div className="atlas-reference-links"><span>ÓRGANOS DE REFERENCIA</span><p>Los modelos anteriores siguen disponibles como vistas independientes.</p>{ORGAN_IDS.map(organ=><SiteLink href={'/anatomia?organ='+organ} key={organ}>{MODEL_DEFINITIONS[organ].title}<CaretRight size={13}/></SiteLink>)}</div>
          </div>}
          <div className="atlas-sidebar-footer"><span className="atlas-status-dot"/>{ready} / {requested.length} módulos cargados</div>
        </aside>
        <section className="atlas-viewport" aria-label="Modelo interactivo del sistema óseo" aria-busy={statuses.some(status=>status.state==='loading'||status.state==='queued')} data-loaded-assets={metrics?.loadedAssets||0} data-mesh-count={metrics?.meshes||0} data-triangle-count={metrics?.triangles||0} data-geometry-bytes={metrics?.geometryBytes||0} data-draw-calls={metrics?.drawCalls||0} data-render-geometries={metrics?.renderGeometries||0} data-render-textures={metrics?.renderTextures||0} data-load-ms={metrics?.loadMs||0}>
          ...
          <div className="atlas-model-title"><span className="atlas-mono">Systema skeletale</span><h2>{current?.name||'Sistema óseo'}</h2><p>{isolated?'Estructura aislada':'Modelo anatómico de referencia'}</p></div>
          ...
          <div className="atlas-load-summary" role="status" aria-live="polite">{errors.length?`${errors.length} módulos no se han podido cargar`:ready<requested.length?`Cargando por regiones · ${ready} de ${requested.length}`:`${metrics?.meshes||catalog.coverage.meshes} piezas · ${mb(requested.reduce((sum,asset)=>sum+asset.bytes,0))} MB de modelos`}</div>
          <div className="atlas-explode-bar"><div><Stack size={17}/><span>Despiece</span><strong>{exploded}%</strong></div><input type="range" min="0" max="100" value={exploded} onChange={event=>setExploded(Number(event.target.value))} aria-label="Separación de piezas"/><select aria-label="Nivel de despiece" value={explodeLevel} onChange={event=>setExplodeLevel(event.target.value as ExplodeLevel)}><option value="systems">1 · Sistemas</option><option value="regions">2 · Regiones</option><option value="structures">3 · Estructuras</option></select></div>
          <div className="atlas-interaction-hint">{explodeLevel==='systems'&&registeredSystems.length===1?<span>El nivel de sistemas requiere más de un sistema corporal compatible.</span>:<>GIRAR <span>arrastrar</span><i/> ZOOM <span>rueda / pellizco</span><i/> MOVER <span>botón derecho</span></>}</div>
        </section>
        <aside className={'atlas-detail '+(panel==='details'?'is-open':'')}>
          ...
          <div className="atlas-detail-content"><div className="atlas-detail-icon"><Bone size={26}/></div><span className="anatomy-eyebrow">{current?'ESTRUCTURA SELECCIONADA':'MODELO DE REFERENCIA'}</span><h2>{current?.name||'Sistema óseo'}</h2><p className="atlas-latin">{current?.latin||terms?.latin||(!current?'Systema skeletale':'')}</p>
            {current&&<><nav className="atlas-hierarchy-path" aria-label="Ubicación anatómica">{path.map(id=><button key={id} onClick={()=>choose(id)}>{index?.byId.get(id)?.name}<CaretRight size={10}/></button>)}</nav><div className="atlas-selected-meta"><span>NOMBRE ANATÓMICO</span><p>{current.anatomicalName}</p><small>{current.sourceId||current.id}</small></div></>}
            <div className="atlas-education"><dl><div><dt>SISTEMA</dt><dd>{current?SYSTEMS.find(system=>system.id===current.systemId)?.name:'Sistema óseo'}</dd></div><div><dt>REGIÓN</dt><dd>{current?regionName(current.regionId):'Esqueleto axial y apendicular'}</dd></div></dl>
              {education?<><span className="atlas-family-note">Ficha de la familia anatómica · {terms?.name||current?.family}</span><h3>Descripción</h3><p>{education.description}</p><h3>Función</h3><p>{education.function}</p><h3>Relaciones anatómicas</h3><p>{education.relations}</p><SiteLink href={education.source.url} target="_blank" rel="noreferrer">{education.source.title}<ArrowSquareOut size={12}/></SiteLink></>:<p>{current?(current.children.length?'Grupo de organización anatómica. Selecciona una estructura para consultar su ficha.':'Ficha específica pendiente de revisión. Se conserva la identificación anatómica del modelo original.'):catalog.coverage.note}</p>}
            ...
            <details className="atlas-coverage"><summary>Cobertura y límites del modelo</summary><p>{catalog.coverage.note}</p>{catalog.coverage.limitations.length>0&&<ul>{catalog.coverage.limitations.map(note=><li key={note}>{note}</li>)}</ul>}<p>{catalog.coverage.structures} estructuras · {catalog.coverage.meshes} piezas · {mb(totalBytes)} MB en {catalog.assets.length} módulos.</p></details>
          </div>
        </aside>
      </div>
...
function Related({current,byId,choose}:{current:AnatomyNode;byId:Map<string,AnatomyNode>;choose:(id:string)=>void}) {
  ...
  return <><h3>En la jerarquía y estructuras relacionadas</h3><p className="atlas-relation-context">Grupos, componentes y referencias anatómicas del catálogo. No es una lista de articulaciones.</p><div className="atlas-related">{items.map(node=><button key={node.id} onClick={()=>choose(node.id)}>{node.name}<CaretRight size={11}/></button>)}</div></>;
}
